...
public class AuthManager : IAuthService
{
    ...
    public async Task<AccessToken> CreateAccessToken(User user)
    {
        ...
        AccessToken accessToken = _tokenHelper.CreateToken(
            user,
            operationClaims.Select(op => (InfoSystem.Core.Security.Entities.OperationClaim<int>)op).ToImmutableList()
        );
        return accessToken;
    }
    ...
    {
        InfoSystem.Core.Security.Entities.RefreshToken<Guid, Guid> newCoreRefreshToken = _tokenHelper.CreateRefreshToken(
            user,
            ipAddress
        );
    ...
    {
        InfoSystem.Core.Security.Entities.RefreshToken<Guid, Guid> coreRefreshToken = _tokenHelper.CreateRefreshToken(
            user,
            ipAddress
        );
